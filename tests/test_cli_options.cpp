...
// C2: --layer-split N. Strict integer parse (reuses parse_u32, which already rejects malformed/
// negative/zero values); requires --split-mode layer; range validation (1..L-1) happens later,
// target-side, once L is known (see qwen3_6_27b::detail::resolve_layer_split_boundary and its
// dedicated unit test in test_shard_map.cpp) -- this file only covers the CLI-level option
// surface: parsing, the default, and the "requires layer mode" hard error.
void check_layer_split() {
    // Omitted: defaults to 0 ("target resolves its own default").
    const ninfer::cli::Options omitted =
        parse_cli({"ninfer", "m.ninfer", "--prompt", "hi", "--split-mode", "layer", "--devices",
                  "0,1"});
    check(omitted.layer_split_boundary == 0, "--layer-split default is not 0");

    // Explicit value parses through under layer mode.
    const ninfer::cli::Options set =
        parse_cli({"ninfer", "m.ninfer", "--prompt", "hi", "--split-mode", "layer", "--devices",
                  "0,1", "--layer-split", "54"});
    check(set.layer_split_boundary == 54, "--layer-split 54 did not set layer_split_boundary");

    // Malformed value is a clean rejection, not a silent fallback.
    bool threw_malformed = false;
    try {
        parse_cli({"ninfer", "m.ninfer", "--prompt", "hi", "--split-mode", "layer", "--devices",
                  "0,1", "--layer-split", "abc"});
    } catch (const std::invalid_argument&) { threw_malformed = true; }
    check(threw_malformed, "--layer-split abc did not throw");

    // Supplying --layer-split without --split-mode layer is a hard error, never a silent ignore.
    bool threw_wrong_mode = false;
    try {
        parse_cli({"ninfer", "m.ninfer", "--prompt", "hi", "--layer-split", "54"});
    } catch (const std::invalid_argument&) { threw_wrong_mode = true; }
    check(threw_wrong_mode, "--layer-split without --split-mode layer did not throw");

    bool threw_tp_mode = false;
    try {
        parse_cli({"ninfer", "m.ninfer", "--prompt", "hi", "--split-mode", "tp", "--tp", "2",
                  "--devices", "0,1", "--layer-split", "54"});
    } catch (const std::invalid_argument&) { threw_tp_mode = true; }
    check(threw_tp_mode, "--layer-split with --split-mode tp did not throw");
}
...
void check_documented() {
    ...
    check(usage.find("--layer-split") != std::string::npos,
          "--layer-split is missing from the CLI usage text");
}
