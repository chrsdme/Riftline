...
int main() {
    ...
    // --- C2: runtime layer-split boundary resolution/ownership (config.h) ---
    {
        using ninfer::targets::qwen3_6_27b::detail::kDefaultLayerSplitBoundary;
        using ninfer::targets::qwen3_6_27b::detail::layer_owner_for_layer;
        using ninfer::targets::qwen3_6_27b::detail::resolve_layer_split_boundary;

        // Default resolution: 0 -> 56.
        if (resolve_layer_split_boundary(0) != static_cast<int>(kDefaultLayerSplitBoundary)) {
            fail("resolve_layer_split_boundary(0) did not resolve to the default (56)");
        }
        // Explicit 56 resolves to the same value (same ownership as default).
        if (resolve_layer_split_boundary(56) != 56) {
            fail("resolve_layer_split_boundary(56) did not round-trip");
        }
        // Invalid: 0 is reserved for "default", not itself a valid explicit boundary distinct
        // from that meaning -- callers that want an explicit boundary must pass a positive value
        // in [1, layers-1]; out-of-range explicit values throw.
        expect_throws([] { (void)resolve_layer_split_boundary(64); },
                      "resolve_layer_split_boundary(64) must reject N >= L");
        expect_throws([] { (void)resolve_layer_split_boundary(65); },
                      "resolve_layer_split_boundary(65) must reject N > L");
        expect_throws([] { (void)resolve_layer_split_boundary(-1); },
                      "resolve_layer_split_boundary(-1) must reject negative N");

        // Owner function at the boundary itself and its neighbors, for both the default (56) and
        // an alternate (54) boundary.
        for (const std::size_t boundary : {std::size_t{56}, std::size_t{54}}) {
            if (layer_owner_for_layer(boundary - 1, boundary) != 0) {
                fail("layer_owner_for_layer(boundary-1) must be owner 0, boundary=" +
                    std::to_string(boundary));
            }
            if (layer_owner_for_layer(boundary, boundary) != 1) {
                fail("layer_owner_for_layer(boundary) must be owner 1, boundary=" +
                    std::to_string(boundary));
            }
        }

        // Owner-local full-attention/GDN counts at two boundaries. Derived from
        // qwen3_6::is_full_attention_layer ((layer+1) % 4 == 0), not guessed:
        //   N=56: owner0 full=14 gdn=42, owner1 full=2 gdn=6  (today's accepted split)
        //   N=54: owner0 full=13 gdn=41, owner1 full=3 gdn=7
        if (TextConfig::owned_full_attention_layers(0, 56) != 14 ||
            TextConfig::owned_gdn_layers(0, 56) != 42 ||
            TextConfig::owned_full_attention_layers(1, 56) != 2 ||
            TextConfig::owned_gdn_layers(1, 56) != 6) {
            fail("owned_*_layers(*, 56) counts regressed from the accepted 56/8 split");
        }
        if (TextConfig::owned_full_attention_layers(0, 54) != 13 ||
            TextConfig::owned_gdn_layers(0, 54) != 41 ||
            TextConfig::owned_full_attention_layers(1, 54) != 3 ||
            TextConfig::owned_gdn_layers(1, 54) != 7) {
            fail("owned_*_layers(*, 54) counts do not match the derived 54/10 split");
        }
    }
