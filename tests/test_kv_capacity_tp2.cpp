...
int main() {
    ...
    // -----------------------------------------------------------------------------------------
    // 4. resolve_kv_capacity_symmetric (three-arg form): automatic mode takes one budget per
    //    device (each caller-measured from that device's own free VRAM) and sizes the single
    //    shared plan to the tightest budget against the ONE shared curve, never averaging. Under
    //    TP2 the curve is symmetric so this is exact; the four-arg overload (case 7) is the exact
    //    form for asymmetric per-device bases.
    // -----------------------------------------------------------------------------------------
    {
        const auto curve = make_tp2_curve(kBytesPerPageInt8Tp2, 16384);
    ...
    // -----------------------------------------------------------------------------------------
    // 7. M2.2b four-arg overload: with SYMMETRIC per-device bases (TP2: both ranks carry the
    //    same reservation) the exact form must be byte-identical to the three-arg form, in both
    //    Automatic and Explicit mode -- the TP2 byte-identity guarantee.
    // -----------------------------------------------------------------------------------------
    {
        const auto curve = make_tp2_curve(kBytesPerPageInt8Tp2, 16384);
        const std::array<std::size_t, 2> per_device_bytes{40ULL * kGiB, 20ULL * kGiB};
        const std::array<std::size_t, 2> bases{curve.minimum_device_reservation_bytes,
                                               curve.minimum_device_reservation_bytes};
        for (const auto& policy : {ninfer::KvCapacityPolicy::automatic(1ULL * kGiB),
                                   ninfer::KvCapacityPolicy::explicit_capacity(262144)}) {
            const auto three =
                ninfer::runtime::resolve_kv_capacity_symmetric(policy, curve, per_device_bytes);
            const auto four = ninfer::runtime::resolve_kv_capacity_symmetric(
                policy, curve, per_device_bytes, bases);
            failures += check(three.main_page_groups == four.main_page_groups &&
                                  three.runtime_reservation_bytes == four.runtime_reservation_bytes &&
                                  three.available_after_weights_bytes ==
                                      four.available_after_weights_bytes &&
                                  three.planned_slack_bytes == four.planned_slack_bytes &&
                                  three.resolved_tokens == four.resolved_tokens,
                              "M2.2b: symmetric per-device bases were not byte-identical to the "
                              "three-arg tp2 resolution");
        }
    }
