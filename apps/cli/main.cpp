...
void print_load_summary(const ninfer::LoadSummary& load, double wall_seconds,
    ...
    if (split_mode == ninfer::SplitMode::Layer) {
        const int n      = load.layer_split_boundary;
        const int l      = load.layer_count;
        const int suffix = l - n;
        print_metric("tensor parallel", "layer split (" + std::to_string(n) + "/" +
                                            std::to_string(suffix) + ")");
        // Full startup diagnostic for the resolved boundary: effective N/L, owner layer ranges,
        // selected devices, and the cheap per-owner full-attention/GDN counts persistent_layout
        // sized state from -- everything needed to confirm which layers landed where without
        // re-deriving it from the boundary by hand.
        print_metric("layer split boundary", std::to_string(n) + " / " + std::to_string(l));
        print_metric("layer split owner0 range", "[0, " + std::to_string(n) + ")");
        print_metric("layer split owner1 range",
                     "[" + std::to_string(n) + ", " + std::to_string(l) + ")");
        print_metric("layer split devices", "gpu" + std::to_string(load.devices[0].device) +
                                                " (owner0), gpu" +
                                                std::to_string(load.devices[1].device) +
                                                " (owner1)");
        print_metric("layer split owner0 full-attn/gdn",
                     std::to_string(load.owner0_full_attention_layers) + " / " +
                         std::to_string(load.owner0_gdn_layers));
        print_metric("layer split owner1 full-attn/gdn",
                     std::to_string(load.owner1_full_attention_layers) + " / " +
                         std::to_string(load.owner1_gdn_layers));
    } else {
        print_metric("tensor parallel",
                     std::to_string(load.tp) + (load.tp > 1 ? " (split)" : ""));
    }
    ...
    for (int rank = 0; rank < load.tp; ++rank) {
        ...
        // Exact byte counts alongside the MiB-rounded figures below -- under `--split-mode layer`
        // owner 0 and owner 1 are sized to DIFFERENT owned layer counts (see
        // PersistentLayout::owner_slot), so a MiB rounding can hide the exact asymmetry a gate
        // check needs to compare against the expected owner-local sizes.
        if (split_mode == ninfer::SplitMode::Layer) {
            print_metric(label + " owner slot", std::to_string(row.owner_slot));
            print_metric(label + " full-attn/gdn layers (exact)",
                         std::to_string(row.full_attention_layers) + " / " +
                             std::to_string(row.gdn_layers));
        }
        print_metric(label + " kv pool bytes (exact)", std::to_string(row.kv_pool_bytes));
        print_metric(label + " gdn state bytes (exact)", std::to_string(row.gdn_state_bytes));
        print_metric(label + " sequence bytes (exact)", std::to_string(row.sequence_bytes));
...
void print_generation_summary(const ninfer::GenerationResult& result,
    ...
    // M3 planned-vs-actual on the PEER device. Printed only when a peer exists; under layer split
    // this is the only place rank 1's own workspace high-water mark is observable, and it is the
    // device where memory is tightest. `capacity_bytes` here is deliberately the same planned
    // figure as rank 0's -- build_workspace_plan is layer-count-, ownership- and
    // architecture-independent -- so a divergence between the two PEAK figures is the interesting
    // signal, not a divergence between the capacities.
    if (memory.workspace_peer.capacity_bytes != 0) {
        print_metric("gpu workspace peak (peer)", format_arena_peak(memory.workspace_peer));
    }
...
int main(int argc, char** argv) {
    try {
        ...
        engine_options.split_mode     = cli.split_mode;
        engine_options.layer_split_boundary = cli.layer_split_boundary;
        ...
        print_load_summary(engine.load_summary(), load_wall, cli.split_mode);
