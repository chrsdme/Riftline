...
SplitMode parse_split_mode(std::string_view text) {
    if (text == "tp") { return SplitMode::TensorParallel; }
    if (text == "layer") { return SplitMode::Layer; }
    throw std::invalid_argument("invalid split-mode: " + std::string(text) +
                                " (expected tp|layer)");
}
...
std::string usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           ...
           "       [--device N] [--tp 1|2] [--split-mode tp|layer] [--layer-split N] [--devices N,N]\n"
           ...
           "not --vision.\n"
           "--split-mode layer uses whole-layer placement across two --devices entries; hybrid is "
           "not exposed in this build.\n"
           "--layer-split N sets the contiguous layer boundary under --split-mode layer: model "
           "layers [0, N) run on the first --devices entry, [N, L) on the second, where L is the "
           "loaded model's layer count. Requires --split-mode layer; omitted defaults to the "
           "target's own boundary (56 of 64 for the qwen3.6/3.8-27b family).\n"
...
Options parse_options(int argc, char** argv) {
    ...
    bool layer_split_explicit    = false;
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--prompt") {
            ...
            options.split_mode = parse_split_mode(value(arg));
        } else if (arg == "--layer-split") {
            options.layer_split_boundary =
                static_cast<int>(parse_u32(value(arg), "layer-split"));
            layer_split_explicit = true;
        } else if (arg == "--devices") {
    ...
    if (options.split_mode == SplitMode::Layer && options.tp != 1) {
        throw std::invalid_argument("--split-mode layer cannot be combined with --tp 2");
    }

    if (layer_split_explicit && options.split_mode != SplitMode::Layer) {
        throw std::invalid_argument("--layer-split requires --split-mode layer");
    }

    if (devices_explicit) {
        const std::size_t expected_devices =
            options.split_mode == SplitMode::Layer ? 2U : static_cast<std::size_t>(options.tp);
        if (options.devices.size() != expected_devices) {
            throw std::invalid_argument(options.split_mode == SplitMode::Layer
                                            ? "--split-mode layer requires exactly two --devices ids"
                                            : "--devices must list exactly --tp device ids");
        }
    ...
    } else if (options.split_mode == SplitMode::Layer) {
        throw std::invalid_argument("--split-mode layer requires --devices A,B");
