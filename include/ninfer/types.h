...
struct EngineOptions {
    ...
    // model across two CUDA devices and requires `devices` to name exactly two distinct ids.
    // Homogeneous pairs retain existing behavior; Phase 1 additionally admits only sm_86 + sm_120
    // for the restricted common Ampere-compatible profile. It is supported by the 27B execution
    // package (`qwen3.6-27b`,
    ...
    int tp = 1;
    SplitMode split_mode = SplitMode::TensorParallel;
    // Contiguous layer-split device boundary: model layers [0, layer_split_boundary) execute on
    // the first selected device, [layer_split_boundary, L) on the second, where L is the loaded
    // variant's layer count. Only meaningful under `split_mode == SplitMode::Layer`; ignored
    // otherwise. 0 means "use the target's default" (resolved once by the target package, e.g.
    // qwen3_6_27b::detail::resolve_layer_split_boundary) -- callers that construct EngineOptions
    // directly (tests, embedders) may leave this at 0 for the default 56/8 split.
    int layer_split_boundary = 0;
...
struct MemorySummary {
    ...
    // Rank 1's (PeerRuntime's) own persistent arena figures, populated only when a peer exists (tp
    // 2 or layer_split). Under `--split-mode layer` these are NOT required to equal the rank-0
    // figures above -- each owner's decoder state is sized to its own owned layer counts, not a
    // shared max. Zero when there is no peer.
    ArenaMemorySummary sequence_peer;
    std::size_t kv_payload_bytes_peer = 0;
    std::size_t gdn_state_bytes_peer  = 0;
    // Rank 1's own workspace arena (M3). `capacity_bytes` is the PLANNED figure, which today is
    // deliberately identical to rank 0's: `build_workspace_plan` is a per-layer-execution
    // high-water mark with no dependency on layer counts, ownership or device architecture, so
    // both devices legitimately plan the same workspace. `peak_bytes` is the MEASURED high-water
    // mark on that device (ArenaMemorySummary::peak_used_bytes) -- the number that makes the planned figure falsifiable, and the one
    // thing the pre-M3 summary could not show for rank 1 at all. Zero when there is no peer.
    ArenaMemorySummary workspace_peer;
};
...
struct DeviceMemoryReport {
    int device                              = -1;
    // Device SLOT (position in --devices, 0 or 1) this row reports on -- NOT the CUDA device id
    // above. Under `--split-mode layer` this is the same slot that determines layer ownership
    // (LoadedModelData::device_slot / layer_owner[]), so a reader can match this row's
    // full_attention_layers/gdn_layers against the owner that actually executes them.
    int owner_slot                          = -1;
    int full_attention_layers               = 0; // this device's OWN full-attention layer count
    int gdn_layers                          = 0; // this device's OWN GDN layer count
    ...
    std::uint64_t workspace_bytes           = 0; // workspace arena capacity (planned)
    // Measured workspace high-water mark on THIS device (M3). Planned vs actual: `workspace_bytes`
    // is what admission reserved, this is what execution actually touched. Under layer split rank 1
    // reads its own PeerRuntime arena rather than rank 0's, so the two rows are independent
    // measurements. Zero if no round has executed yet (the summary is taken at load).
...
struct LoadSummary {
    ...
    // Populated only under SplitMode::Layer: the RESOLVED layer-split boundary (0 -> the target's
    // default, already resolved to its concrete value here) and the model's total layer count, so
    // a caller can print "layer split (prefix/suffix)" without re-deriving the default itself.
    // Zero at TP/single-device.
    int layer_split_boundary = 0;
    int layer_count          = 0;
    // Per-owner full-attention/GDN layer counts under the resolved boundary (cheap to compute from
    // layer_split_boundary/layer_count plus the model's fixed hybrid-attention interval; carried
    // here so the CLI can print them without re-deriving the partition). All zero outside
    // SplitMode::Layer.
