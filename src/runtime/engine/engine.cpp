...
void require_supported_tp_features(const EngineOptions& options) {
    if (options.split_mode != SplitMode::TensorParallel) { return; }
...
// construct. ExecutionContext validates ids and admits only the homogeneous or Phase-1 pair.
std::vector<int> resolve_execution_device_ids(const EngineOptions& options) {
    ...
    // layer_split_boundary is meaningless outside SplitMode::Layer (see EngineOptions). The CLI
    // catches this for the --layer-split flag (apps/cli/options.cpp), but EngineOptions is also a
    // public library surface a caller can construct directly (tests, embedders) -- reject the same
    // combination here rather than silently ignoring the field for non-CLI callers.
    if (options.split_mode != SplitMode::Layer && options.layer_split_boundary != 0) {
        throw std::invalid_argument(
            "EngineOptions.layer_split_boundary requires split_mode == SplitMode::Layer");
    }
    if (options.split_mode == SplitMode::Layer) {
        if (options.tp != 1) {
            throw std::invalid_argument("--split-mode layer cannot be combined with --tp 2");
        }
        if (options.devices.size() != 2) {
            throw std::invalid_argument("--split-mode layer requires exactly two devices");
        }
        if (options.speculative.backend != SpeculativeBackend::None) {
            throw std::invalid_argument("--split-mode layer supports greedy text only tonight");
        }
        if (options.enable_vision) {
            throw std::invalid_argument("--split-mode layer does not support Vision");
        }
        if (options.use_cuda_graph) {
            throw std::invalid_argument("--split-mode layer requires --no-cuda-graph");
        }
        // M5.3: under an asymmetric layer split the two owners hold different numbers of
        // full-attention layers, so their per-page-group KV strides differ (the KV pool sizes as
        // owned_full_attention_layers x pages x const -- see decoder_state.cpp plan_cache and
        // layouts_impl.h make_sequence_planner_impl). The shared SequenceCapacityCurve is a single
        // affine (base, stride) pair and cannot represent a max of two differently-sloped lines, so
        // Automatic KV growth is not modelled here. Growth requires max_concurrency > 1; reject that
        // combination rather than silently sizing KV as if concurrency were 1. No shipping surface
        // can reach it today (the CLI has no --max-concurrency; serve cannot select layer split),
        // but EngineOptions is a public library surface -- which is exactly what the comment above
        // says this block is for. Per-slot capacity curves are the upgrade path (deferred).
        if (options.max_concurrency > 1) {
            throw std::invalid_argument(
                "--split-mode layer does not support max_concurrency > 1");
        }
...
void validate_phase1_heterogeneous_options(const EngineOptions& options,
                                           const ExecutionContext& execution) {
    if (!execution.phase1_heterogeneous()) { return; }
    if (options.speculative.backend != SpeculativeBackend::None) {
        throw std::invalid_argument(
            "Phase-1 heterogeneous TP supports greedy text only; MTP/speculation is disabled");
    }
    if (options.enable_vision) {
        throw std::invalid_argument("Phase-1 heterogeneous TP does not support Vision");
    }
    if (options.use_cuda_graph) {
        throw std::invalid_argument(
            "Phase-1 heterogeneous TP requires CUDA graphs to be disabled");
    }
    if (options.kv_cache != KvCacheStorage::BFloat16 &&
        options.kv_cache != KvCacheStorage::Int8Group64) {
        throw std::invalid_argument("Phase-1 heterogeneous TP supports only BF16 or INT8-G64 KV");
    }
}
...
public:
    ...
    explicit Impl(EngineOptions engine_options)
        ...
        validate_phase1_heterogeneous_options(options, execution);
        ...
        // construct_target resolves layer_split_boundary internally (0 -> target default) and
        // reports the canonical value on `load`; mirror it back onto the options this Impl keeps
        // so a caller reading Engine::options() after construction sees the resolved value instead
        // of the raw 0-means-default request it passed in.
        if (options.split_mode == SplitMode::Layer) {
            options.layer_split_boundary = load.layer_split_boundary;
        }
        executor              = std::visit(
...
std::vector<DebugTensorSnapshot> Engine::debug_last_parity_trace() const {
    if (impl_ == nullptr) { throw std::logic_error("Engine is moved from"); }
    return std::visit(
        [](const auto& executor) -> std::vector<DebugTensorSnapshot> {
            using Executor = std::remove_cvref_t<decltype(executor)>;
            if constexpr (std::is_same_v<Executor, std::monostate>) {
                throw std::logic_error("concurrent Engine executor is unavailable");
            } else {
                return executor->debug_last_parity_trace();
            }
        },
        impl_->executor);
}

void Engine::debug_enable_parity_trace(bool enabled) {
    if (impl_ == nullptr) { throw std::logic_error("Engine is moved from"); }
    std::visit(
        [enabled](const auto& executor) {
            using Executor = std::remove_cvref_t<decltype(executor)>;
            if constexpr (std::is_same_v<Executor, std::monostate>) {
                ...
                executor->debug_enable_parity_trace(enabled);
            }
        },
        impl_->executor);
}
