...
public:
    ...
    [[nodiscard]] std::vector<DebugTensorSnapshot> debug_last_parity_trace() const {
        std::scoped_lock lock(execution_mutex_);
        const std::span<const DebugTensorSnapshot> trace = instance_.program->last_parity_trace();
        return std::vector<DebugTensorSnapshot>(trace.begin(), trace.end());
    }

    void debug_enable_parity_trace(bool enabled) {
        std::scoped_lock lock(execution_mutex_);
        instance_.program->enable_parity_trace(enabled);
    }
