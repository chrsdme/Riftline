...
// Resident-CTA capacity of one cooperative MMA instantiation on the current
// device: occupancy (min over Full/Predicated token variants, which may differ
// in register count) times the physical SM count. 0 on any probe failure.
template <class Geometry, int SplitK, int Warps = kBf16GdnWarps>
std::int32_t query_cooperative_device_ctas() noexcept {
    ...
    int per_sm       = std::numeric_limits<int>::max();
    const auto probe = [&](auto full_tokens) -> bool {
        constexpr bool FullTokens = decltype(full_tokens)::value;
        auto* kernel = bf16_gdn_gating_proj_gemm_mma_kernel<Geometry, SplitK, FullTokens, Warps>;
        if (cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel),
                                 cudaFuncAttributeMaxDynamicSharedMemorySize,
                                 kSmemBytes) != cudaSuccess) {
            return false;
        }
        int blocks = 0;
        if (cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocks, kernel, Warps * 32,
                                                          kSmemBytes) != cudaSuccess ||
            blocks <= 0) {
            return false;
        }
        per_sm = std::min(per_sm, blocks);
        return true;
    };
    if (!probe(std::true_type{}) || !probe(std::false_type{})) { return 0; }
    return per_sm * sm_count;
}

std::int32_t compute_cooperative_device_ctas(int split_k, bool family_35) noexcept {
    if (family_35) {
        switch (split_k) {
        case 32:
            return query_cooperative_device_ctas<Bf16Gdn35Geometry, 32, 8>();
        case 16:
            return query_cooperative_device_ctas<Bf16Gdn35Geometry, 16, 8>();
        case 8:
            return query_cooperative_device_ctas<Bf16Gdn35Geometry, 8, 8>();
        case 4:
            return query_cooperative_device_ctas<Bf16Gdn35Geometry, 4, 8>();
        case 2:
            return query_cooperative_device_ctas<Bf16Gdn35Geometry, 2, 8>();
        default:
            return 0;
        }
    }
    switch (split_k) {
    case 8:
        return query_cooperative_device_ctas<Bf16Gdn27Geometry, 8, 8>();
    case 4:
        return query_cooperative_device_ctas<Bf16Gdn27Geometry, 4>();
    case 2:
        return query_cooperative_device_ctas<Bf16Gdn27Geometry, 2>();
    default:
        return 0;
    }
}
...
std::int32_t bf16_gdn_gating_cooperative_device_ctas(const int split_k,
                                                     const bool family_35) noexcept {
    int split_idx = -1;
    switch (split_k) {
    case 2:
        split_idx = 0;
    ...
    // 0 = unprobed, -1 = probe failed (capacity 0), >0 = resident CTAs. A racy
    // double probe is idempotent, so relaxed atomics suffice.
    static std::atomic<std::int32_t> cache[2][5][kMaxCachedDevices];
    if (device >= kMaxCachedDevices) { return compute_cooperative_device_ctas(split_k, family_35); }
    auto& slot        = cache[family_35 ? 1 : 0][split_idx][device];
    std::int32_t ctas = slot.load(std::memory_order_relaxed);
    if (ctas == 0) {
        ctas = compute_cooperative_device_ctas(split_k, family_35);
        if (ctas <= 0) { ctas = -1; }
        slot.store(ctas, std::memory_order_relaxed);
    }
    return ctas < 0 ? 0 : ctas;
}
