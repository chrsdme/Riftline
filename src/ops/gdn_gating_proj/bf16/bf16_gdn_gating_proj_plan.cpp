...
bool cooperative_27_grid_is_resident(Bf16GdnGatingScheduleId schedule, std::int32_t cols) noexcept {
    // Three 16-row tiles per BN128 token tile. The resident-CTA envelope is measured
    // occupancy times SM count on the CUDA device current at resolve time (0 when the
    // probe fails, rejecting the cooperative route so resolution falls back toward the
    // non-cooperative MmaUnsplit schedule).
    return cooperative_grid_is_resident(
        schedule, cols, 128, 3,
        bf16_gdn_gating_cooperative_device_ctas(schedule_split_k(schedule), false));
}

bool cooperative_35_grid_is_resident(Bf16GdnGatingScheduleId schedule, std::int32_t cols) noexcept {
    // Two 16-row tiles per BN64 token tile; same physical-device envelope as above.
    return cooperative_grid_is_resident(
        schedule, cols, 64, 2,
        bf16_gdn_gating_cooperative_device_ctas(schedule_split_k(schedule), true));
}
...
std::size_t route_capacity(const std::array<RouteSpec, N>& routes, const Bf16GdnGatingProblem& base,
    ...
    for (const RouteSpec& route : routes) {
        ...
        // Size for the table candidate rather than the resolved plan: legality is
        // physical-device dependent and resolution only ever lowers SplitK from the
        // candidate, so the candidate's bytes are a safe workspace bound on any device.
        const std::int32_t split_k = schedule_split_k(route.schedule);
        if (split_k > 1) {
            maximum = std::max(maximum,
                               checked_partial_bytes(base.heads, split_k, endpoint));
        }
    }
    return maximum;
}
...
std::size_t bf16_gdn_norm_gating_capacity_workspace_bytes(std::int32_t heads,
    ...
    if (heads == 32 && input_rows == 2048 && min_cols <= 16) {
        // Device-independent bound for the fused split32 route (control partials plus
        // norm partials); whether the fused route is legal is decided per device at
        // resolve time and only ever needs less than this.
        const std::int32_t fused_cols = std::min<std::int32_t>(max_cols, 16);
        maximum                       = std::max(
            maximum, checked_partial_bytes(heads, 32, fused_cols) +
                                       static_cast<std::size_t>(32) * fused_cols * sizeof(float));
    }
    return maximum;
}
