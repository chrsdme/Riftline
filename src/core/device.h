...
struct ExecutionContext {
    ...
    // DeviceContext's own constructor. A heterogeneous pair is admitted only for the explicit
    // Phase-1 common Ampere profile: sm_86 plus sm_120.
    explicit ExecutionContext(const std::vector<int>& device_ids);

    [[nodiscard]] bool phase1_heterogeneous() const noexcept;
