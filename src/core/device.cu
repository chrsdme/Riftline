...
ExecutionContext::ExecutionContext(const std::vector<int>& device_ids) {
    ...
    if (tp == 2) {
        ...
        const int sm0 = p0.major * 10 + p0.minor;
        const int sm1 = p1.major * 10 + p1.minor;
        const bool homogeneous = sm0 == sm1;
        const bool phase1_pair = (sm0 == 86 && sm1 == 120) || (sm0 == 120 && sm1 == 86);
        if (!homogeneous && !phase1_pair) {
            throw std::runtime_error(
                "ExecutionContext supports only homogeneous pairs or the Phase-1 sm_86 + sm_120 "
                "pair (device " +
...
bool ExecutionContext::phase1_heterogeneous() const noexcept {
    if (tp != 2) { return false; }
    const int sm0 = dev[0]->sm();
    const int sm1 = dev[1]->sm();
    return (sm0 == 86 && sm1 == 120) || (sm0 == 120 && sm1 == 86);
}
