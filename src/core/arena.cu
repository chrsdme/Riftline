...
// Failure-path only: attempted allocation size plus a fresh cudaMemGetInfo free reading, so an
// admission-adjacent OOM can be diagnosed from the exception text alone (M2 calibration need).
std::string cuda_alloc_error_message(const char* prefix, cudaError_t err, std::size_t requested_bytes) {
    std::size_t free_bytes  = 0;
    std::size_t total_bytes = 0;
    const cudaError_t info_err = cudaMemGetInfo(&free_bytes, &total_bytes);
    std::string message = cuda_error_message(prefix, err) + " requested_bytes=" +
                          std::to_string(requested_bytes);
    if (info_err == cudaSuccess) {
        message += " cudaMemGetInfo_free_bytes=" + std::to_string(free_bytes);
    }
    return message;
}
...
DeviceBuffer::DeviceBuffer(std::size_t size_bytes) : bytes(size_bytes) {
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(cuda_alloc_error_message("cudaMalloc failed", err, bytes));
    }
    p = ptr;
}
...
DeviceArena::DeviceArena(std::size_t capacity_bytes) {
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(cuda_alloc_error_message("cudaMalloc failed", err, capacity_bytes));
    }
