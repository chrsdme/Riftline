...
struct Package {
    ...
    // No layer-split support on this variant (see resolve_options); 0 means LoadSummary's
    // layer_split_boundary/layer_count diagnostics are never populated for it.
    static constexpr int layer_count             = 0;
    ...
    // This variant has no layer-split boundary (no owned_* layer partitioning, see
    // layouts_impl.h's owned_*_or_full SFINAE fallback), so resolution is the identity: options
    // pass through unchanged. Mirrors qwen3_6_27b::Package::resolve_options so construct_registered
    // can call Target::resolve_options uniformly for either target.
    [[nodiscard]] static EngineOptions resolve_options(const EngineOptions& options);
    // No layer-split support on this variant (see resolve_options); always all-zero.
