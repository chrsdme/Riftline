...
// kDefaultLayerSplitBoundary / layer_owner_for_layer() live in impl/config.h (single source of
// truth, shared with the generic runtime's per-device state sizing via Variant::TextConfig).
...
// `placement` is the resolved layer-split placement (boundary from
// detail::resolve_layer_split_boundary, endpoint owner from Package::manual_placement or an
// explicit planner candidate); meaningless when `layer_split` is false. M5.0: this is the ONLY
// input the shard resolver derives ownership from (config.h owner_for_object) -- no env read here.
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
                               qwen3_6::StartupFeatures features, int tp = 1,
                               bool layer_split = false,
                               PlacementSpec placement = PlacementSpec{
                                   .boundary = static_cast<int>(kDefaultLayerSplitBoundary)});

// M4a manual endpoint selection (NINFER_ENDPOINT_DEVICE, read once per process). Kept as the
// target's name for the shared targets::manual_endpoint_owner_slot; only the manual runtime path
// (Package::manual_placement) consumes it. Planner candidates carry an explicit slot instead.
[[nodiscard]] int endpoint_owner_device();
...
public:
    LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized,
                    int tensor_parallel = 1, bool layer_split = false,
                    PlacementSpec placement = PlacementSpec{
                        .boundary = static_cast<int>(kDefaultLayerSplitBoundary)});
    ...
    int tp = 1;
    bool layer_split = false;
    // M5.0: the same spec the shard resolver placed the bytes with; build_device_view reads
    // ownership ONLY from here (ModelView::endpoint_owner_slot, layer_owner[]).
...
public:
    Impl(WeightsProfile weights_profile_in, BindingPlan plan,
         artifact::MaterializedArtifact materialized, int tensor_parallel, bool layer_split,
         PlacementSpec placement = PlacementSpec{
             .boundary = static_cast<int>(kDefaultLayerSplitBoundary)})
        : weights_profile(weights_profile_in),
          data(std::move(plan), std::move(materialized), tensor_parallel, layer_split, placement) {
    }
