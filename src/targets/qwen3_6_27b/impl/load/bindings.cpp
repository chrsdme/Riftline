...
// M4: which device owns the output endpoint group (`text/final_norm` + `text/output_head`) under
// layer split. Pre-M4 there was no choice to make -- `layer_owner` below returns 0 for every object
// that is not `text/layers/N/...`, so both endpoints landed on device 0 by FALLTHROUGH rather than
// by any placement decision. That default costs GPU0 1,350,860,800 bytes for the output head alone
// (plus 10,240 for the norm) on the device that is already the tightest under a prefix-heavy split.
//
// Only the OUTPUT group is selectable. `text/token_embedding` deliberately stays on device 0: layer
// split is a contiguous prefix/suffix, so device 0 always owns layer 0 and the embedding is
// consumed before it -- moving it would ADD a prologue hop that no path has today. The output group
// is the opposite case: device 1 always owns the LAST layer, so placing the output there REMOVES
// the existing final-hidden 1->0 pull-back. `final_norm` must travel with the head or the hidden
// would cross twice (1->0 to norm, 0->1 to project).
//
// Selected by environment variable rather than a plumbed EngineOptions field, matching the existing
// NINFER_ALLOCATOR_RESERVE_BYTES calibration-knob convention: M4's charter is to make placement
// EXPLICIT and MEASURABLE, not to choose it automatically (that is M5). Unset or unparsable keeps
// the pre-M4 behaviour byte-for-byte.
// STATUS (M4b): placement AND execution are implemented. `NINFER_ENDPOINT_DEVICE=1` relocates the
// bytes, every admission figure follows exactly (M4a), and the runtime executes the endpoint on
// the owner: `build_device_view` records the decision in `ModelView::endpoint_owner_slot`, and the
// single runtime helper `endpoint_project` (text_context_impl.h) runs final norm + output head on
// the owning card and copies only the required logits (and the normalized hidden the caller
// needs) back to the primary, where sampling stays. Runtime code never re-reads this variable.
// See doc/claude-phase2/M4B_ENDPOINT_EXECUTION.md.
// M5.0: the env read lives in targets::manual_endpoint_owner_slot (shared with the admission
// printer); ownership itself is decided by config.h owner_for_object from a PlacementSpec, so the
// shard resolver below, the runtime-view condition in build_device_view and the analytical planner
// all consume ONE explicit spec. The manual path resolves the env into that spec exactly once
// (Package::manual_placement); candidate evaluation never touches the environment.

artifact::ShardPlacement layer_placement(std::string_view object, PlacementSpec placement) {
    artifact::ShardPlacement placement_out;
    placement_out.axis         = artifact::ShardAxis::SingleDevice;
    placement_out.owner_device = owner_for_object(placement, object);
    return placement_out;
}

} // namespace

int endpoint_owner_device() { return targets::manual_endpoint_owner_slot(); }
...
ArtifactLoadPlan bind_artifact(artifact::Binder& binder, WeightsProfile weights_profile,
                               qwen3_6::StartupFeatures features, int tp, bool layer_split,
    ...
    const int expected_devices = layer_split ? 2 : tp;
    if (binder.device_count() != expected_devices) {
        throw std::invalid_argument("qwen3_6_27b: binder was built for " +
                                    ...
                                    " device(s) but bind_artifact was asked for " +
                                    std::to_string(expected_devices));
    }
    if (layer_split) {
        if (features.vision || features.mtp() || features.dflash() ||
            features.optimized_proposal()) {
            throw std::invalid_argument(
                "qwen3_6_27b: layer split supports greedy text weights only");
        }
        binder.set_shard_resolver(
            [placement](std::string_view name) { return layer_placement(name, placement); });
    } else if (tp > 1) {
        if (features.vision) {
...
LoadedModelData::LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized,
                                 int tensor_parallel, bool layer_split_in,
                                 PlacementSpec placement_in)
    : backing(std::move(materialized)), tp(tensor_parallel), layer_split(layer_split_in),
    ...
    if (tp == 2 || layer_split) { build_device_view(plan, 1, runtime_peer.emplace()); }
}
...
void LoadedModelData::build_device_view(const BindingPlan& plan, int device,
    ...
    runtime.layer_split   = layer_split;
    runtime.device_slot   = device;
    // M4b/M5.0: the same PlacementSpec the shard resolver used to place the bytes (stored on this
    // object, never re-read from the environment); outside layer split the endpoint is always on
    // slot 0.
    runtime.endpoint_owner_slot = layer_split ? placement.endpoint_owner_slot : 0;
    for (std::size_t layer = 0; layer < runtime.layer_owner.size(); ++layer) {
        runtime.layer_owner[layer] = owner_for_layer(placement, layer);
    }
    ...
    if (!layer_split || device == 0) {
        token_embedding = materialized_weight(backing, plan.token_embedding, 248320, 5120, device);
    }
    ...
    for (std::size_t layer = 0; layer < kTextLayers; ++layer) {
        ...
        if (source.is_full_attention) {
            ...
            if (layer_split && runtime.layer_owner[layer] != device) { continue; }
            target.input_norm =
    ...
    // M4: must agree with `layer_owner`'s endpoint decision above -- the shard resolver decides
    // which device the bytes are MATERIALIZED on, this decides which device builds a runtime view
    // of them. A disagreement would leave the owner without a view or build a view over memory that
    // was never populated. Outside layer split this is unchanged (`device == 0`). M4b: the runtime
    // reads `runtime.endpoint_owner_slot` (set above from the same cached decision) to pick the
    // executing card, so the non-owner's `final_norm`/`output_head` stay empty and unused.
    if (!layer_split || device == runtime.endpoint_owner_slot) {
