...
struct ExecutionCore {
    ...
    const LayerPeerCore* layer_peer = nullptr;
    std::vector<DebugTensorSnapshot>* parity_trace = nullptr;
};
...
[[nodiscard]] inline std::optional<LayerExecution>
layer_execution(const ExecutionCore& execution) {
    if (execution.layer_peer == nullptr) { return std::nullopt; }
    const LayerPeerCore& peer = *execution.layer_peer;
    return LayerExecution{.device         = peer.device,
                          .weights        = peer.model,
                          .work           = peer.work,
                          .state          = peer.linear_attention,
                          .io             = peer.io,
                          .prefill_hidden = peer.prefill_hidden,
                          .batch_kv       = peer.text_cache};
}

struct PrefillContext {
    ExecutionCore execution;
    qwen3_6::PagedKVCacheView text_kv;
    qwen3_6::PagedKVCacheView text_kv_peer;
