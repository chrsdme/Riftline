...
// Definition lives in layouts_impl.h (after build_sequence_candidate, which it calls for the
// explicit-page-group overload; that helper is file-local to layouts_impl.h's anonymous
// namespace, and layouts_impl.h is included after this file from instantiate.h).
...
template <>
std::span<const DebugTensorSnapshot> Program<Variant>::last_parity_trace() const noexcept {
    return impl_->last_parity_trace();
}

template <>
void Program<Variant>::enable_parity_trace(bool enabled) {
    impl_->enable_parity_trace(enabled);
}
