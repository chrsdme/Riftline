...
struct PeerRuntime {
    ...
    WorkspaceArena work;
    // This device's own kv/GDN byte totals, captured from the SAME persistent layout the
    // DecoderState below was constructed from (see ProgramImplCore's identically-named members) --
    // reported by ProgramImplCore::memory_summary() as the peer figures.
...
public:
    ...
    std::optional<schedule::LayerPeerCore> layer_core;
    std::unique_ptr<qwen3_6::DecoderState> decoder;
    std::optional<GdnReplayRecords> replay_records;
    ...
    std::vector<DebugTensorSnapshot> parity_trace;
...
private:
    ...
    [[nodiscard]] qwen3_6::PagedKVCacheView text_kv_view_peer(const SequenceState& sequence) const;
