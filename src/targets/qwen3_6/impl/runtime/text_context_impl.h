...
template <class Tap>
void tap_begin(Tap& tap, const Tensor& value, int device, cudaStream_t stream) {
    if constexpr (requires { tap.begin(value, device, stream); }) {
        tap.begin(value, device, stream);
    } else {
        tap.begin(value);
    }
}
...
template <class Tap>
void tap_layer(Tap& tap, int layer, const Tensor& value, int device, cudaStream_t stream) {
    if constexpr (requires { tap.capture_layer(layer, value, device, stream); }) {
        tap.capture_layer(layer, value, device, stream);
    } else {
        tap.capture_layer(layer, value, stream);
    }
}

template <class Tap>
void tap_capture(Tap& tap, std::string name, const Tensor& value, int device, cudaStream_t stream) {
    if constexpr (requires { tap.capture(std::move(name), device, value, stream); }) {
        tap.capture(std::move(name), device, value, stream);
    }
}
...
// M4b: THE endpoint execution path. Runs final norm + output head on whichever device SLOT owns the
// endpoint group (`ModelView::endpoint_owner_slot`, decided once at load) and leaves the results the
// caller needs on the CALLER's device/stream:
//   x           [hidden, columns] BF16, contiguous, on the caller's device (post-last-layer residual,
//               or an already-normalized hidden when `apply_norm` is false)
//   hidden_out  nullable [hidden, columns] on the caller's device; receives the NORMALIZED hidden
//               when `apply_norm` (required then). Consumers -- `continuation_hidden_store` ->
//               `tail_hidden` -> zero-suffix `sample_from_hidden`, the prefill `prefill_hidden`
//               window read by `copy_tail`, the "final_norm" parity tap -- are all post-norm, so
//               this MUST be the post-norm tensor, never the raw input. It is load-bearing for
//               prefix reuse correctness, not an optional convenience copy.
//   logits_out  [vocab, head_columns] on the caller's device; receives the projection of the LAST
//               `head_columns` columns (0 = norm only, e.g. a non-final prefill chunk).
// Owner-local (the default `NINFER_ENDPOINT_DEVICE` unset/0 path, TP2, single device): exactly the
// pre-M4b op sequence on the caller's stream, op-for-op, so the default stays byte-identical.
// Peer-owned: the same sync -> copy -> sync idiom `run_layers` uses for its layer boundary (no
// events, no P2P assumption, driver host-staged D2D). Temporaries live in the peer's workspace
// arena under a scope taken strictly AFTER `run_layers` has returned and released its own scopes.
// The final `cudaStreamSynchronize(stream)` is MANDATORY, not defensive: the return copies read
// arena-scoped peer temporaries on the caller's stream, and the scope guard below releases them
// when this block exits. Keep the sync inside the same block as the scope guard.
void endpoint_project(int primary_device, const LoadedModelData& primary_weights,
                      const LayerExecution* layer, const Tensor& x, Tensor* hidden_out,
                      Tensor* logits_out, std::int32_t head_columns, bool apply_norm, float rms_eps,
                      cudaStream_t stream) {
    const std::int32_t columns = x.ne[1];
    if (columns <= 0 || head_columns < 0 || head_columns > columns) {
        throw std::invalid_argument("endpoint projection column window is out of range");
    }
    if (apply_norm && hidden_out == nullptr) {
        throw std::logic_error("endpoint projection needs a normalized-hidden destination");
    }
    if (head_columns > 0 && logits_out == nullptr) {
        throw std::logic_error("endpoint projection needs a logits destination");
    }
    const auto project = [&](const Tensor& normalized, const Weight& head, Tensor& logits,
                             cudaStream_t s) {
        if (head_columns == columns) {
            ops::linear(normalized, head, logits, s);
        } else {
            Tensor last = normalized.slice(1, columns - head_columns, head_columns);
            ops::linear(last, head, logits, s);
        }
    };

    const bool peer_owns = layer != nullptr && primary_weights.layer_split &&
                           primary_weights.endpoint_owner_slot != primary_weights.device_slot;
    if (!peer_owns) {
        const Tensor* normalized = &x;
        if (apply_norm) {
            ops::rmsnorm(x, primary_weights.final_norm, rms_eps, true, *hidden_out, stream);
            normalized = hidden_out;
        }
        if (head_columns > 0) { project(*normalized, primary_weights.output_head, *logits_out, stream); }
        return;
    }

    const LoadedModelData& peer_weights = *layer->weights;
    if (peer_weights.endpoint_owner_slot != peer_weights.device_slot ||
        peer_weights.final_norm.data == nullptr || peer_weights.output_head.payload == nullptr) {
        throw std::logic_error("layer-split peer does not hold the endpoint it is asked to execute");
    }
    if (!x.is_contiguous() || (hidden_out != nullptr && !hidden_out->is_contiguous()) ||
        (head_columns > 0 && !logits_out->is_contiguous())) {
        throw std::invalid_argument("endpoint projection across devices needs contiguous tensors");
    }
    // `x` was produced on the caller's stream; make it visible before the peer reads it.
    CUDA_CHECK(cudaStreamSynchronize(stream));
    {
        ScopedCudaDevice peer_device(layer->device->device);
        cudaStream_t peer_stream = layer->device->stream;
        // M5.3 transient-memory contract. The three temporaries below (px, the separate pn when
        // apply_norm, and pl) all come from the PEER's own workspace arena and all coexist until
        // `endpoint_scope` rewinds it, so the peak here is
        //     hidden_buffers * 10,240 * columns + 496,640 * head_columns   bytes
        // (hidden 5120 BF16 = 10,240 B/col, two buffers when a distinct pn is allocated and one when
        // pn aliases px; logits 248,320 BF16 = 496,640 B/col). `build_workspace_plan` does not
        // simulate this stage, so it is accounted for by fitting inside the arena that plan already
        // sizes, which rests on two facts:
        //
        //   1. `columns` and `head_columns` are never both large. On the prefill chunk loop --
        //      the only path where `columns` reaches prefill_chunk -- `head_columns` is
        //      `is_last ? 1 : 0`. The paths where head_columns == columns (ordinary decode, verify)
        //      are bounded by batch, and layer split pins batch to 1 (engine.cpp rejects
        //      max_concurrency > 1 under layer split, M5.3) and rejects MTP/speculation outright.
        //      So the 496,640 B/col vocabulary term never multiplies a chunk-scale factor.
        //   2. Given that, the peak is 20,480 * chunk + 496,640, while the planned arena measures
        //      176,712 B/col with no chunk-independent floor (measured on the real artifact at
        //      57/7: 22,619,136 B at --prefill-chunk 128 and 180,953,088 B at 1024). The arena is
        //      ~8.6x the endpoint's per-column cost, so the margin is wide and WIDENS with chunk
        //      size: 7.25x at chunk 128, 8.43x at 1024.
        //
        // This call also runs strictly after run_layers has released its own scopes, so it reuses
        // arena space the prefill body already required rather than adding to a live peak.
        // ninfer_endpoint_transient_test pins both facts against those measured arena figures, with
        // the endpoint side computed from TextConfig constants. Note the comparison must be against
        // the ARENA (a high-water mark over stages), not against any single stage's cost: the
        // smallest stage alone runs only ~384 B/col above this one, which the 496,640 B logits
        // constant would exceed below ~1293 columns.
        auto endpoint_scope      = layer->work->scope();
        Tensor px = layer->work->alloc(DType::BF16, {x.ne[0], columns});
        CUDA_CHECK(cudaMemcpyAsync(px.data, x.data, x.bytes(), cudaMemcpyDeviceToDevice,
                                   peer_stream));
        Tensor pn = px;
        if (apply_norm) {
            pn = layer->work->alloc(DType::BF16, {x.ne[0], columns});
            ops::rmsnorm(px, peer_weights.final_norm, rms_eps, true, pn, peer_stream);
        }
        Tensor pl;
        if (head_columns > 0) {
            pl = layer->work->alloc(DType::BF16, {logits_out->ne[0], head_columns});
            project(pn, peer_weights.output_head, pl, peer_stream);
        }
        CUDA_CHECK(cudaStreamSynchronize(peer_stream));

        CUDA_CHECK(cudaSetDevice(primary_device));
        if (head_columns > 0) {
            CUDA_CHECK(cudaMemcpyAsync(logits_out->data, pl.data, pl.bytes(),
                                       cudaMemcpyDeviceToDevice, stream));
        }
        if (hidden_out != nullptr) {
            CUDA_CHECK(cudaMemcpyAsync(hidden_out->data, pn.data, pn.bytes(),
                                       cudaMemcpyDeviceToDevice, stream));
        }
        // MANDATORY (see the contract above): `pl`/`pn` are released when `endpoint_scope` goes
...
void ParityTraceTap::capture(std::string name, int device, const Tensor& value,
                             cudaStream_t stream) {
    if (snapshots == nullptr) { return; }
    if (value.dtype != DType::BF16 || !value.is_contiguous() || value.data == nullptr) {
        throw std::logic_error("parity trace only captures contiguous BF16 tensors");
    }
    DebugTensorSnapshot snapshot;
    snapshot.name   = std::move(name);
    snapshot.device = device;
    for (int i = 0; i < 4; ++i) { snapshot.shape[static_cast<std::size_t>(i)] = value.ne[i]; }
    snapshot.bf16.resize(static_cast<std::size_t>(value.numel()));
    const CurrentDevice restore;
    CUDA_CHECK(cudaSetDevice(device));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    CUDA_CHECK(cudaMemcpy(snapshot.bf16.data(), value.data, value.bytes(), cudaMemcpyDeviceToHost));
    snapshots->push_back(std::move(snapshot));
}

void ParityTraceTap::begin(const Tensor& value, int device, cudaStream_t stream) {
    capture("embedding", device, value, stream);
}

void ParityTraceTap::capture_layer(int layer, const Tensor& value, int device,
                                   cudaStream_t stream) {
    char name[16];
    std::snprintf(name, sizeof(name), "layer_%02d", layer);
    capture(name, device, value, stream);
}
...
TextContext::TextContext(
    ...
    : ctx_(ctx), weights_(weights), work_(work), kv_(kv), mtp_kv_(mtp_kv), state_(state), io_(io),
      ...
      tp_(tp), layer_(layer) {
    if (weights_.layer_split) {
        bool found_full = false;
        bool found_gdn  = false;
        for (int layer_idx = 0; layer_idx < kCfg.n_layers && !(found_full && found_gdn);
            ++layer_idx) {
            if (weights_.layer_owner[static_cast<std::size_t>(layer_idx)] != weights_.device_slot) {
                continue;
            }
            if (ModelConfig::is_full(layer_idx)) {
                if (!found_full) {
                    state_full_offset_ = ModelConfig::full_idx(layer_idx);
                    found_full         = true;
                }
            } else if (!found_gdn) {
                state_gdn_offset_ = ModelConfig::gdn_idx(layer_idx);
                found_gdn         = true;
            }
        }
    }
...
void TextContext::bind() {
    ...
    if (layer_ != nullptr) {
        const LoadedModelData& peer = *layer_->weights;
        for (int layer = 0; layer < kCfg.n_layers; ++layer) {
            if (peer.layer_owner[static_cast<std::size_t>(layer)] != peer.device_slot) {
                continue;
            }
            if (ModelConfig::is_full(layer)) {
                const std::size_t fidx = static_cast<std::size_t>(ModelConfig::full_idx(layer));
    ...
    for (int layer = 0; layer < kCfg.n_layers; ++layer) {
        if (weights_.layer_split &&
            weights_.layer_owner[static_cast<std::size_t>(layer)] != weights_.device_slot) {
            continue;
        }
        if (ModelConfig::is_full(layer)) {
...
void TextContext::proposal_argmax(const Tensor& hidden, Tensor& logits, Tensor& proposal_tokens) {
    ...
    if (proposal_head_ != nullptr) {
        ...
        endpoint_project(ctx_.device, weights_, layer_, hidden, nullptr, &output_logits, T,
                         /*apply_norm=*/false, kCfg.rms_eps, ctx_.stream);
...
void TextContext::ordinary_decode_batch(const Tensor& ids, const Tensor& cache_positions,
    ...
    work_.reset();
    {
        ...
        endpoint_project(ctx_.device, weights_, layer_, x, &hidden, &logits, batch,
                         /*apply_norm=*/true, kCfg.rms_eps, stream);
    }
    work_.reset();
}
...
void TextContext::target_verify_batch_impl(const Tensor& ids, const Tensor& cache_positions,
    ...
    work_.reset();
    {
        ...
        if constexpr (Tap::enabled) { tap_begin(tap, x, ctx_.device, stream); }
        run_layers(x, Phase::Verify, tap);
        ...
        endpoint_project(ctx_.device, weights_, layer_, x, &flat_hidden, &flat_logits, columns,
                         /*apply_norm=*/true, kCfg.rms_eps, stream);
...
void TextContext::attn_mix(const FullLayerW& w, Tensor& x, int fidx, Phase ph) {
    ...
    if (active_sequence_batch_ != 0) {
        ...
        ops::gqa_attention(q_batch, k_batch, v_batch, position_batch, valid, kv_table_rows,
                           kAttnScale, batch_text_kv_->batch_layer_view(fidx - state_full_offset_),
                           *active_gqa_envelope_, work_, a_batch, s);
    } else {
        ops::gqa_attention(qn, kn, v, cache_positions, Tensor{}, kv_table_rows, kAttnScale,
                           batch_text_kv_->batch_layer_view(fidx - state_full_offset_),
...
void TextContext::gdn_mix(const GdnLayerW& w, Tensor& x, int gidx, Phase ph) {
    ...
    if (ph == Phase::Verify) {
        ...
        Tensor& conv_states =
            state_.conv.at(static_cast<std::size_t>(gidx - state_gdn_offset_));
        const Tensor valid = active_valid_columns_ != nullptr ? *active_valid_columns_ : Tensor{};
        ...
        Tensor conv_state = state_.conv_slot(static_cast<std::uint32_t>(gidx - state_gdn_offset_),
    ...
    if (ph == Phase::Verify) {
        Tensor& recurrent_states =
            state_.recurrent.at(static_cast<std::size_t>(gidx - state_gdn_offset_));
        const std::int32_t width = active_sequence_width_;
        Tensor q_batch =
        ...
        Tensor recurrent_state = state_.recurrent_slot(
            static_cast<std::uint32_t>(gidx - state_gdn_offset_), linear_state_current_slot_);
...
void TextContext::run_layers(Tensor& x, Phase ph, Tap& tap) {
    if (layer_ != nullptr) {
        for (int layer = 0; layer < kCfg.n_layers; ++layer) {
            if (weights_.layer_owner[static_cast<std::size_t>(layer)] != weights_.device_slot) {
                continue;
            }
            if (ModelConfig::is_full(layer)) {
                ...
                }
                {
                    ...
                    if constexpr (Tap::enabled) { tap_layer(tap, layer, x, ctx_.device, ctx_.stream); }
                }
            } else {
        ...
        Tensor peer_x = layer_->prefill_hidden->slice(1, 0, x.ne[1]).view({x.ne[0], x.ne[1]});
        if constexpr (Tap::enabled) {
            tap_capture(tap, "boundary_0_to_1_src", x, ctx_.device, ctx_.stream);
        }
        CUDA_CHECK(cudaStreamSynchronize(ctx_.stream));
        {
            ScopedCudaDevice peer_device(layer_->device->device);
            CUDA_CHECK(cudaMemcpyAsync(peer_x.data, x.data, x.bytes(), cudaMemcpyDeviceToDevice,
                                       layer_->device->stream));
            CUDA_CHECK(cudaStreamSynchronize(layer_->device->stream));
            if constexpr (Tap::enabled) {
                tap_capture(tap, "boundary_0_to_1_dst", peer_x, layer_->device->device,
                            layer_->device->stream);
            }
            layer_->work->reset();
            Tensor peer_cache_positions;
            Tensor peer_rope_positions;
            Tensor peer_kv_table_rows;
            Tensor peer_linear_state_slots;
            Tensor peer_valid_columns;
            Tensor peer_backend_kv_table_rows;
            const auto copy_peer_i32 = [&](const Tensor* source, Tensor& target) -> const Tensor* {
                if (source == nullptr) { return nullptr; }
                if (source->dtype != DType::I32 || !source->is_contiguous()) {
                    throw std::logic_error("layer-split peer control tensor must be contiguous I32");
                }
                target = layer_->work->alloc(DType::I32,
                                             {source->ne[0], source->ne[1], source->ne[2],
                                              source->ne[3]});
                CUDA_CHECK(cudaMemcpyAsync(target.data, source->data, source->bytes(),
                                           cudaMemcpyDeviceToDevice, layer_->device->stream));
                return &target;
            };
            const Tensor* peer_cache =
                copy_peer_i32(active_cache_positions_, peer_cache_positions);
            const Tensor* peer_rope = copy_peer_i32(active_rope_positions_, peer_rope_positions);
            const Tensor* peer_rows = copy_peer_i32(active_kv_table_rows_, peer_kv_table_rows);
            const Tensor* peer_slots =
                copy_peer_i32(active_linear_state_slots_, peer_linear_state_slots);
            const Tensor* peer_valid = copy_peer_i32(active_valid_columns_, peer_valid_columns);
            const Tensor* peer_backend_rows =
                copy_peer_i32(active_backend_kv_table_rows_, peer_backend_kv_table_rows);
            TextContext peer_card(*layer_->device, *layer_->weights, *layer_->work,
                                  rope_frequency_, layer_->kv, *layer_->state, *layer_->io,
                                  *layer_->prefill_hidden, prefill_chunk_, text_kv_base_, {},
                                  layer_->batch_kv, nullptr, nullptr, nullptr);
            peer_card.active_cache_positions_       = peer_cache;
            peer_card.active_rope_positions_        = peer_rope;
            peer_card.active_kv_table_rows_         = peer_rows;
            peer_card.active_linear_state_slots_    = peer_slots;
            peer_card.active_valid_columns_         = peer_valid;
            peer_card.active_backend_kv_table_rows_ = peer_backend_rows;
            peer_card.active_gqa_envelope_          = active_gqa_envelope_;
            peer_card.active_sequence_batch_        = active_sequence_batch_;
            peer_card.active_sequence_width_        = active_sequence_width_;
            peer_card.set_linear_state_slots(linear_state_current_slot_,
                                             linear_state_rewrite_checkpoint_slot_);
            peer_card.run_layers(peer_x, ph, tap);
            // B1.2 fix: unlike the 0->1 boundary above (which syncs the producer's own stream
            // before handing off) and unlike TP2's allgather_rows (which links producer/consumer
            // streams with cudaEventRecord/cudaStreamWaitEvent), this 1->0 pull-back had no
            // dependency at all on layer_->device->stream, where peer_card.run_layers() just
            // enqueued its work. The cudaMemcpyAsync below reads peer_x on ctx_.stream with no
            // guarantee that write has landed -- a real cross-device race in the inference path,
            // not a debug-only one. Synchronizing the producer stream here (same pattern as line
            // ~1209) closes it.
            CUDA_CHECK(cudaStreamSynchronize(layer_->device->stream));
            if constexpr (Tap::enabled) {
                tap_capture(tap, "boundary_1_to_0_src", peer_x, layer_->device->device,
                            layer_->device->stream);
            }
        }
        CUDA_CHECK(cudaSetDevice(ctx_.device));
        CUDA_CHECK(
            cudaMemcpyAsync(x.data, peer_x.data, x.bytes(), cudaMemcpyDeviceToDevice, ctx_.stream));
        CUDA_CHECK(cudaStreamSynchronize(ctx_.stream));
        if constexpr (Tap::enabled) {
            tap_capture(tap, "boundary_1_to_0_dst", x, ctx_.device, ctx_.stream);
        }
        return;
    }
...
TextContext::prefill_impl(std::span<const int> ids, const TextPrefill* text_prefill,
    ...
    for (; t0 < T;) {
...

        {
            ...
            if constexpr (Tap::enabled) { tap_begin(tap, x, ctx_.device, s); }
            ...
            // M4b: the whole `len`-column window is normalized into `xf` (the prefill_hidden
            // window `copy_tail` and the parity tap read), and only the LAST column is projected,
            // both on the endpoint owner; results land on this device.
            Tensor logits = matrix_window(io_.logits, 1);
            endpoint_project(ctx_.device, weights_, layer_, x, &xf, &logits, is_last ? 1 : 0,
                             /*apply_norm=*/true, kCfg.rms_eps, s);
            if constexpr (Tap::enabled) { tap_capture(tap, "final_norm", xf, ctx_.device, s); }

            if (is_last) {
                if constexpr (Tap::enabled) { tap_capture(tap, "logits", logits, ctx_.device, s); }
    ...
    for (int layer = 0; layer < kCfg.n_layers; ++layer) {
        if (ModelConfig::is_full(layer)) {
            ...
            }
            {
                ...
                if constexpr (Tap::enabled) {
                    tap_layer(tap, layer, x[0], ctx_.device, ctx_.stream);
                    tap_capture(tap, "tp2_rank1_layer_" + std::to_string(layer), x[1],
                                tp_->device->device, tp_->device->stream);
                }
            }
        } else {
...
PrefillChunkResult TextContext::prefill_impl_tp2(std::span<const int> ids,
    ...
    tp_->work->reset();
    {
        ...
        if constexpr (Tap::enabled) {
            tap_begin(tap, x[0], ctx_.device, ctx_.stream);
            tap_capture(tap, "tp2_rank1_embedding", x[1], tp_->device->device,
                        tp_->device->stream);
        }
        ...
        if constexpr (Tap::enabled) {
            tap_capture(tap, "final_norm", xf[0], ctx_.device, ctx_.stream);
            tap_capture(tap, "tp2_rank1_final_norm", xf[1], tp_->device->device,
                        tp_->device->stream);
        }
        ...
        if (is_last) {
            ...
            if constexpr (Tap::enabled) {
                tap_capture(tap, "logits", logits, ctx_.device, ctx_.stream);
                tap_capture(tap, "tp2_rank1_logits", peer_logits, tp_->device->device,
                            tp_->device->stream);
            }
...
PrefillChunkResult TextContext::prefill_impl_tp2(std::span<const int> ids,
    ...
    NullTap tap;
    return prefill_impl_tp2(ids, text_prefill, finalize_at_end, tap);
}
