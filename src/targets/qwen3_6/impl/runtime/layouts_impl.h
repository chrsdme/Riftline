...
// Owner-local layer counts (see qwen3_6_27b::detail::TextConfig::owned_full_attention_layers /
// owned_gdn_layers) are only defined on variants with a layer-split boundary. `persistent_layout`
// below is compiled once per variant with a fixed concrete `TextConfig`, not a template parameter,
// so calling straight through would fail to compile for every variant without that boundary. These
// two small function templates carry the SFINAE-friendly `if constexpr` discard: variants without
// the method get the model's full layer count back (today's behavior, byte-identical), and only
// qwen3_6_27b's real branch is ever compiled with a genuinely different value.
template <class Cfg>
std::int32_t owned_full_attention_layers_or_full(int owner, std::size_t boundary) {
    if constexpr (requires { Cfg::owned_full_attention_layers(owner, boundary); }) {
        return Cfg::owned_full_attention_layers(owner, boundary);
    } else {
        (void)owner;
        (void)boundary;
        return Cfg::full_attention_layers();
    }
}

template <class Cfg>
std::int32_t owned_gdn_layers_or_full(int owner, std::size_t boundary) {
    if constexpr (requires { Cfg::owned_gdn_layers(owner, boundary); }) {
        return Cfg::owned_gdn_layers(owner, boundary);
    } else {
        (void)owner;
        (void)boundary;
        return Cfg::gdn_layers();
    }
}
...
// `owner` is the device SLOT (0 or 1, matching LoadedModelData::device_slot / layer_owner[] -- NOT
// a CUDA ordinal) this layout is being sized for. Only consulted under `plan.layer_split`; ignored
// (today's shared full-model sizing) otherwise, so the non-layer-split path stays byte-identical
// regardless of which owner is requested.
PersistentLayout persistent_layout(const SequencePlanImpl& plan, int owner) {
    ...
    // A boundary of 0 under layer_split means the caller (loader) and this planner would silently
    // disagree about ownership -- 0 makes owner 0 empty and owner 1 own every layer, which is
    // full-model sizing wearing a "sized correctly" costume, not a caught error. The canonical
    // value is resolved exactly once (qwen3_6_27b::detail::resolve_layer_split_boundary) and must
    // never be 0 by the time it reaches here; catch the two sources disagreeing before it corrupts
    // memory sizing instead of after.
    if (plan.layer_split && plan.layer_split_boundary == 0) {
        throw std::invalid_argument(
            "sequence plan layer_split_boundary is 0 under layer_split -- the boundary must be "
            "resolved before reaching persistent_layout (loader/planner disagreement)");
    }
    // Under layer split, `plan.tp` is forced to 1 (each device runs a disjoint layer range, not a
    // head/channel shard of every layer -- see program_impl.h's `tp(layer_split ? 1 : tp)`), so
    // each device needs KV/GDN state for exactly ITS OWN owned layer counts, not a shared
    // max-of-both-owners figure. `owner` (the device slot this call is sizing for) selects which
    // owner's counts apply; the caller (below, at persistent_layout's two call sites) invokes this
    // once per slot and keeps the two resulting layouts separate (SequencePlanImpl::persistent /
    // persistent_peer). TP (tp == 2) keeps today's full-layer-count sizing untouched, since every
    // device executes all 64 layers there -- `owner` is not consulted on that path at all.
    // `TextConfig::owned_full_attention_layers`/`owned_gdn_layers` only exist on variants that
    // define a layer-split boundary (currently qwen3_6_27b). This file is compiled once per
    // Qwen3.6-family variant with `TextConfig` a fixed concrete type (see instance.h's
    // NINFER_QWEN36_VARIANT macro), not a template parameter, so a bare `if constexpr` here would
    // still require the discarded branch to be well-formed. Routing through the real function
    // templates below restores the SFINAE-friendly discard: variants without an owner-count method
    // fall straight through to today's full-layer-count sizing, unchanged, and their TextConfig is
    // never touched by this change.
    const std::int32_t text_full_attention_layers =
        plan.layer_split ? owned_full_attention_layers_or_full<TextConfig>(
                               owner, plan.layer_split_boundary)
                          : TextConfig::full_attention_layers();
    const std::int32_t text_gdn_layers =
        plan.layer_split ? owned_gdn_layers_or_full<TextConfig>(owner, plan.layer_split_boundary)
    ...
    out.decoder = qwen3_6::plan_decoder_state(
        builder, qwen3_6::DecoderStateSpec{
                     .full_attention_layers     = text_full_attention_layers,
    ...
    out.owner_slot = plan.layer_split ? owner : -1;
    return out;
}
...
std::unique_ptr<SequencePlanImpl> build_sequence_candidate(const SequencePlanningInputs& inputs,
    ...
    impl->tp                  = inputs.tp;
    impl->layer_split         = inputs.layer_split;
    impl->layer_split_boundary = inputs.layer_split_boundary;
    impl->kv_dtype            = inputs.kv_dtype;
    impl->kv_quant_group      = inputs.kv_quant_group;
    impl->persistent          = persistent_layout(*impl, 0);
    // Owner 1's own layout, sized to its own owned layer counts (not owner 0's), only under layer
    // split. `impl->persistent` above stays owner 0's -- and the ONLY layout outside layer split --
    // so every non-layer-split reader of `plan.persistent` is untouched by this addition.
    if (impl->layer_split) { impl->persistent_peer = persistent_layout(*impl, 1); }
    ...
    // M2.2: the reservation this plan reports is the WORST OWNING DEVICE's, not owner 0's.
    // `device_reservation_bytes` becomes `curve.minimum_device_reservation_bytes` (see
    // make_sequence_planner_impl below) and is what `resolve_kv_capacity_symmetric` charges
    // against the bottleneck device's free memory. Pre-M2.2 it summed `impl->persistent.bytes`
    // -- owner 0's layout -- so under `--split-mode layer` a device-1-heavy split (e.g. 12/52,
    // where owner 1 owns 52 of 64 layers) was sized against owner 0's much smaller persistent
    // footprint. That is an under-charge: it can only under-reject, never over-reject, which is
    // why it never produced a false rejection -- only a missing one.
    //
    // Taking the max over OWNING SLOTS keeps one shared curve, which is what the resolver and
    // finalize_sequence_plan_impl's affine-equality check both require. A max of two affine
    // functions is itself affine ONLY when the two share a per-page-group stride -- otherwise it
    // is piecewise, and one probed increment cannot represent it.
    //
    // CORRECTED BY M5.3. An earlier version of this comment asserted that the stride is always
    // identical on both owners ("KV page geometry is not divided across devices, so only the BASE
    // differs by owned layer mix"), citing a measurement at split 35/29 where both slots showed a
    // stride of exactly 1,081,600 bytes. The page COUNT is indeed shared, but it multiplies an
    // OWNER-LOCAL layer count: the Paged KV pool builds 2 planes per layer (4 at INT8, see
    // decoder_state.cpp plan_cache) and every plane tensor carries the physical page count as a
    // dimension (paged_kv_cache.cpp plan_paged_kv_pool), so
    //     stride(owner) = owned_full_attention_layers(owner, boundary) x const.
    // With kHybridAttentionInterval == 4 the two owners hold equal full-attention counts at only
    // four boundaries, 32..35 -- and 35/29 sits inside that set, so that measurement was a
    // property of the boundary tested, not a structural law. At 57/7 the ownership is 14:2 and the
    // observed KV payload ratio is 7:1.
    //
    // The equal-stride condition is therefore no longer ASSUMED here: make_sequence_planner_impl
    // computes both owners' strides and throws when they differ, so any curve that survives
    // construction genuinely satisfies the property this max relies on. See that check, and
    // doc/claude-phase2/M5_3_FEASIBILITY_HARDENING.md.
    //
    // The max is taken over the WHOLE reservation total rather than the persistent term alone.
    // Today that is byte-identical -- `workspace.capacity`, `request_transient_capacity_bytes`
    // and `graph_allowance_bytes` are common-route values, identical on both devices (workspace
    // measures 180,953,088 on both devices at every split observed; the other two are 0 under
    // layer split, which forces --no-cuda-graph). It is written as a whole-total max so that when
    // M3 makes workspace per-device (its actual target), the correct device-max behaviour is
    // already in place here and M3 need only change what those terms ARE, not how they combine.
    //
    // Outside layer split `persistent_peer` is empty and `persistent_for(1)` returns `persistent`,
    // so this collapses to exactly the pre-M2.2 value: TP2 and single-device stay byte-identical.
    const auto reservation_for = [&](int slot) {
        return checked_add(
            checked_add(checked_add(impl->persistent_for(slot).bytes, impl->workspace.capacity,
    ...
    impl->device_reservation_bytes = std::max(reservation_for(0), reservation_for(1));
    return impl;
}
...
make_sequence_planner_impl(DeviceContext& device, const EngineOptions& options,
    ...
    SequencePlanningInputs inputs{
        ...
        .tp             = options.tp,
        .layer_split    = options.split_mode == SplitMode::Layer,
        .layer_split_boundary = static_cast<std::size_t>(options.layer_split_boundary),
    };
    ...
    if (minimum_pages < maximum_pages) {
        ...
        // M5.3: the curve below is ONE affine (base, stride) pair, and `device_reservation_bytes` is
        // the MAX over owning slots (see the comment on that field's computation above). A max of two
        // affine functions is affine only while one of them dominates throughout: if the two owners'
        // strides differ, the max is piecewise and the single stride probed here (min -> min+1)
        // extrapolates incorrectly at larger page counts. That error would surface far downstream as
        // the "not affine" logic_error in finalize_sequence_plan_impl and as the post-finalize
        // equality check in registry.cpp -- both of which report the symptom, not the cause. Detect
        // it here instead, where the assumption is actually made.
        //
        // The per-slot stride is proportional to that owner's full-attention layer count: the Paged
        // KV pool builds 2 planes per layer (4 when the KV cache is INT8) and every plane tensor
        // carries the physical page count as a dimension, so its bytes are
        // owned_full_attention_layers(owner, boundary) x pages x const (decoder_state.cpp plan_cache,
        // paged_kv_cache.cpp plan_paged_kv_pool). With kHybridAttentionInterval == 4 the two owners
        // hold equal full-attention counts at only four boundaries (32..35); M2.2's measurement that
        // found identical strides was taken at 35/29, inside that set, and generalized from it.
        //
        // Self-limiting, so no explicit layer_split condition is needed: at tp 2 every device
        // executes all 64 layers (the owner argument is never consulted), and outside layer split
        // `persistent_for(1)` returns owner 0's layout -- both give equal strides. Only an
        // asymmetric layer split can trip this.
        //
        // Deliberately a throw rather than pinning the curve to its minimum point: silently
        // returning concurrency-1 capacity to a caller who asked for more would be fail-silent, not
        // fail-closed. Modelling growth for asymmetric splits needs per-slot curves carried through
        // resolution, which is a larger change than this milestone's scope allows.
        const auto slot_reservation = [](const SequencePlanImpl& plan, int slot) {
            return checked_add(
                checked_add(plan.persistent_for(slot).bytes, plan.workspace.capacity,
                            "Qwen3.6 per-slot reservation overflows size_t"),
                checked_add(plan.request_transient_capacity_bytes, plan.graph_allowance_bytes,
                            "Qwen3.6 per-slot reservation overflows size_t"),
                "Qwen3.6 per-slot reservation overflows size_t");
        };
        const std::size_t stride_slot0 =
            slot_reservation(*adjacent, 0) - slot_reservation(*planner->minimum, 0);
        const std::size_t stride_slot1 =
            slot_reservation(*adjacent, 1) - slot_reservation(*planner->minimum, 1);
        if (stride_slot0 != stride_slot1) {
            throw std::logic_error(
                "Qwen3.6 per-owner KV capacity strides differ under layer split (slot 0 " +
                std::to_string(stride_slot0) + " vs slot 1 " + std::to_string(stride_slot1) +
                " bytes per page group); the shared capacity curve cannot model growth for an "
                "asymmetric split");
        }
...
template <>
DeviceLayoutPreview SequencePlanner<Variant>::layout_preview(
    int slot, std::optional<std::uint32_t> main_page_groups) const noexcept {
    if (impl_ == nullptr || impl_->minimum == nullptr) { return DeviceLayoutPreview{}; }
    // No explicit page count, or it matches the minimum point already built: read the existing
    // minimum-point plan, byte-identical to the pre-M2.1 behaviour.
    if (!main_page_groups.has_value() || *main_page_groups == impl_->curve.minimum_main_page_groups) {
        const auto& plan   = *impl_->minimum;
        const auto& layout = plan.persistent_for(slot);
        return DeviceLayoutPreview{
            .available                         = true,
            .persistent_bytes                  = layout.bytes,
            .persistent_kv_payload_bytes       = layout.kv_payload_bytes,
            .workspace_capacity_bytes          = plan.workspace.capacity,
            .request_transient_capacity_bytes  = plan.request_transient_capacity_bytes,
            .graph_allowance_bytes             = plan.graph_allowance_bytes,
        };
    }
    // Exact preview at the RESOLVED page count (post-materialize `--kv-capacity auto` admission,
    // M2.1). Reuses the SAME build_sequence_candidate byte-accounting path finalize_sequence_plan_impl
    // calls -- never a second one. That path throws (checked_add/checked_mul overflow, or a
    // positive-count precondition) rather than being noexcept, so a failure here degrades to
    // `available = false` instead of propagating through this noexcept method.
    try {
        const auto candidate = detail::NINFER_QWEN36_RUNTIME_NS::build_sequence_candidate(
            impl_->inputs, *main_page_groups);
        const auto& layout = candidate->persistent_for(slot);
        return DeviceLayoutPreview{
            .available                         = true,
            .persistent_bytes                  = layout.bytes,
            .persistent_kv_payload_bytes       = layout.kv_payload_bytes,
            .workspace_capacity_bytes          = candidate->workspace.capacity,
            .request_transient_capacity_bytes  = candidate->request_transient_capacity_bytes,
            .graph_allowance_bytes             = candidate->graph_allowance_bytes,
        };
