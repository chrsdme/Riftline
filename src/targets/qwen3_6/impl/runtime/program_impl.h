...
// Under layer split, `plan.persistent_for(peer_model.device_slot)` is peer's OWN owner-local
// layout (persistent_peer, sized by owner-local layer counts), not the shared `plan.persistent`
// every non-layer-split caller still reads. `peer_model.device_slot` is the same field
// bindings.cpp populated layer_owner[] from (build_device_view(plan, 1, runtime_peer.emplace())),
// not a CUDA ordinal -- see program_impl.h's ProgramImplCore ctor, which passes `*peer_model` (the
// device-1 view) here. Every reference below reads the SAME selected layout so the arena size and
// the bindings placed into it can never disagree.
PeerRuntime::PeerRuntime(DeviceContext& peer_device, const LoadedModelData& peer_model,
                         const SequencePlanImpl& plan)
    : device(peer_device), model(peer_model),
      persistent(plan.persistent_for(peer_model.device_slot).bytes),
      workspace_storage(plan.workspace.capacity),
      work(DeviceSpan{workspace_storage.base(), workspace_storage.capacity()}),
      kv_payload_bytes(plan.persistent_for(peer_model.device_slot).kv_payload_bytes),
      gdn_state_bytes(linear_attention_state_bytes(
          plan.persistent_for(peer_model.device_slot).decoder.linear_attention)) {
    const PersistentLayout& layout = plan.persistent_for(model.device_slot);
    // Picking the wrong layout silently undersizes the arena -- an OOB, not an exception -- so this
    // is checked, not assumed. `layout.owner_slot` is set by persistent_layout() to the SAME
    // integer `owner` it was called with (layouts_impl.h); comparing it to `model.device_slot`
    // catches selection bugs at construction rather than at a stray write past the arena boundary.
    if (plan.layer_split && layout.owner_slot != model.device_slot) {
        throw std::logic_error(
            "peer persistent layout owner slot does not match the peer device slot");
    }
    const DeviceSpan backing = persistent.alloc_bytes(layout.bytes, 256);
    decoder = std::make_unique<qwen3_6::DecoderState>(backing, layout.decoder);
    if (layout.replay_records) { replay_records.emplace(backing, *layout.replay_records); }
    io             = qwen3_6::RoundState(backing, layout.round);
    prefill_hidden = layout.prefill_hidden.bind(backing);
    token_counts   = layout.token_counts.bind(backing);
}
...
ProgramImplCore::ProgramImplCore(const LoadedModelData& model_in,
    ...
    : model(model_in), execution(execution_in), device(execution_in.primary()),
      tp(model_in.layer_split ? 1 : execution_in.tp), layer_split(model_in.layer_split),
    ...
    if (layer_split) {
        if (peer_model == nullptr || plan.tp != 1 || execution.tp != 2) {
            throw std::invalid_argument("Qwen3.6 layer split requires a tp1 plan and two devices");
        }
    } else if ((tp == 2) != (peer_model != nullptr) || (tp == 2) != (plan.tp == 2)) {
    ...
    if (tp == 2 || layer_split) {
        ...
        if (tp == 2) { peer_events.emplace(execution); }
        if (tp == 2 && plan.use_cuda_graph) {
    ...
    // The primary device is always device slot 0 (see ProgramImplCore's `device(execution.primary())`
    // and bindings.cpp's `build_device_view(plan, 0, runtime)`), so `plan.persistent` -- owner 0's
    // layout under layer split, the ONLY layout outside it -- is always the right one; this asserts
    // that invariant rather than assuming it, since picking the wrong layout silently undersizes
    // the arena instead of throwing where the mistake was made.
    if (layer_split && plan.persistent.owner_slot != model.device_slot) {
        throw std::logic_error(
            "primary persistent layout owner slot does not match the primary device slot");
    }
    ...
    if (peer) {
        ...
        CUDA_CHECK(cudaSetDevice(previous));
        if (tp == 2) {
            peer_core.emplace(schedule::TpPeerCore{
                .execution        = &execution,
                .events           = &*peer_events,
                .device           = &peer->device,
                .model            = &peer->model,
                ...
                .replay_records   = peer->replay_records ? &*peer->replay_records : nullptr,
                .mtp_host_ingress = mtp_peer_host_ingress,
                .graph_bridge     = graph_bridge ? &*graph_bridge : nullptr});
        } else if (layer_split) {
            layer_core.emplace(schedule::LayerPeerCore{
                .device           = &peer->device,
                .model            = &peer->model,
                .work             = &peer->work,
                .linear_attention = &peer->decoder->linear_attention,
...
ProgramImplCore::text_kv_view_peer(const SequenceState& sequence) const {
    if (!peer || !sequence.kv || !sequence.kv->text_peer) { return {}; }
    return peer->decoder->text_kv.execution_view(*sequence.kv->text_peer);
}
...
void ProgramImplCore::prepare_graphs() {
    ...
    const auto execution_core = [&] {
        return schedule::ExecutionCore{device,
                                       ...
                                       layer_core ? &*layer_core : nullptr,
                                       parity_trace_enabled ? &parity_trace : nullptr};
    };
...
//
// B1.2 race fix: debug_last_round_logits_bf16() (concurrent_executor.h) reads logits_capture on
// the host with no sync of its own -- its only documented contract is "after generate()/wait()
// returns". This async D2H is the last write to that host buffer, so it must be complete before
// this function returns, not merely enqueued-in-order behind whatever future call happens to
// synchronize device.stream next. A stream sync right after the copy makes that true unconditionally
// and is provably inert on the inference compute path: it is inside the logits_capture_enabled
// branch, which is off by default and used only by the debug/parity harness.
...
void ProgramImplCore::enable_parity_trace(bool enabled) {
    parity_trace_enabled = enabled;
    parity_trace.clear();
    if (!enabled) { parity_trace.shrink_to_fit(); }
}
...
MemorySummary ProgramImplCore::memory_summary() const noexcept {
    ...
    if (peer) {
        out.sequence_peer = ArenaMemorySummary{peer->persistent.capacity(), peer->persistent.used(),
                                               peer->persistent.peak_used()};
        out.kv_payload_bytes_peer = peer->kv_payload_bytes;
        out.gdn_state_bytes_peer  = peer->gdn_state_bytes;
        // M3: the peer's OWN workspace arena. Pre-M3 the summary reported rank 0's workspace for
        // both rows, so GPU1's actual high-water mark was unobservable -- planned workspace could
        // not be checked against execution on the device where memory is tightest.
        out.workspace_peer = ArenaMemorySummary{peer->workspace_storage.capacity(),
                                                peer->work.used(), peer->work.peak_used()};
    }
    return out;
}
...
void ProgramImplCore::reset_memory_peaks() noexcept {
    ...
    work.reset_peak();
    workspace_logical_peak_bytes = 0;
    // M3: the peer's arenas were never reset here, so once its workspace peak became observable a
    // caller resetting between phases would have kept reading a stale rank-1 high-water mark.
    if (peer) {
        peer->persistent.reset_peak();
        peer->work.reset_peak();
    }
}
