...
PrefillChunkResult prefill_text_chunk(
    ...
    std::optional<LayerExecution> layer = layer_execution(state.execution);
    if (layer) { layer->kv = state.text_kv_peer; }
    if (tp) {
    ...
    TextContext card(state.execution.device, state.execution.model, state.execution.work,
                     ...
                     tp ? &*tp : nullptr, layer ? &*layer : nullptr);
    ...
    if (state.execution.parity_trace != nullptr) {
        state.execution.parity_trace->clear();
        ParityTraceTap tap{state.execution.parity_trace};
        return card.prefill_chunk(prompt, state.text_kv_base, nominal_length, finalize_at_end,
                                  tap);
    }
    return card.prefill_chunk(prompt, state.text_kv_base, nominal_length, finalize_at_end);
...
void sample_from_hidden(PrefillContext& state, const Tensor& hidden, std::int32_t absolute_position,
    ...
    // M4b: `hidden` is the retained POST-NORM tail hidden (decode/prefill store it normalized), so
    // only the output head runs here, on the endpoint owner; the logits land on this device.
    const std::optional<LayerExecution> layer = layer_execution(state.execution);
    endpoint_project(state.execution.device.device, state.execution.model,
                     layer ? &*layer : nullptr, hidden, nullptr, &logits, 1,
                     /*apply_norm=*/false, TextConfig::rms_epsilon, state.execution.device.stream);
