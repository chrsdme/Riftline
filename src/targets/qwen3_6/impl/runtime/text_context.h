...
struct ParityTraceTap {
    static constexpr bool enabled = true;
    std::vector<DebugTensorSnapshot>* snapshots = nullptr;

    void capture(std::string name, int device, const Tensor& value, cudaStream_t stream);
    void begin(const Tensor& value, int device, cudaStream_t stream);
    void capture_layer(int layer, const Tensor& value, int device, cudaStream_t stream);
};
...
public:
    ...
    TextContext(DeviceContext& ctx, const LoadedModelData& weights, WorkspaceArena& work,
                ...
                const LayerExecution* layer                = nullptr);
    ~TextContext();
    ...
    [[nodiscard]] PrefillChunkResult prefill_chunk(std::span<const int> full_ids,
                                                   std::uint32_t begin,
                                                   std::uint32_t nominal_length,
                                                   bool finalize_at_end, ParityTraceTap& tap);
    [[nodiscard]] PrefillChunkResult prefill_chunk(std::span<const int> full_ids,
                                                   std::uint32_t begin,
                                                   std::uint32_t nominal_length,
...
private:
    ...
    // Owner-local KV/GDN state index offsets: how many of this device's OWN family-local indices
    // (full_idx/gidx, which run 0..n_full()-1 / 0..n_gdn()-1 globally) are smaller than the first
    // one this device owns. persistent_layout() sizes each device's decoder state to that device's
    // OWN owned layer counts (see layouts_impl.h), so a device whose owned layers don't start at
    // family index 0 -- i.e. device 1 under the accepted 56/8 layer split -- must subtract this
    // before indexing into that state (batch_layer_view / recurrent_slot). This offset derivation
    // depends only on weights_.layer_owner[]/full_idx/gdn_idx (global, family-local indices), never
    // on the allocated array size, so it is unaffected by which owner-local count the allocation
    // was sized to. Zero whenever layer_split is off (TP or single-device: full_idx/gidx already
    // are the state index) or for the owner whose layers start at 0 (device 0 here, since it owns
    // the layer-index prefix). Computed once in the constructor from weights_.layer_owner[] -- no
    // hardcoded boundary.
    int state_full_offset_ = 0;
    int state_gdn_offset_  = 0;
    ...
    const LayerExecution* layer_                 = nullptr;
