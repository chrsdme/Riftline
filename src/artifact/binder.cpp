...
void Binder::materialize_on_device(ObjectHandle handle) {
    ...
    for (int device = 0; device < materialization_.device_count; ++device) {
        ...
        if (placement.axis == ShardAxis::SingleDevice) {
            if (!ranges.empty()) {
                throw ArtifactError("a single-device placement must carry no shard ranges: " +
                                    std::string(tensor->name));
            }
            if (placement.owner_device < 0 ||
                placement.owner_device >= materialization_.device_count) {
                throw ArtifactError("single-device placement names an invalid owner: " +
                                    std::string(tensor->name));
            }
            if (device == placement.owner_device) { place(handle, device, tensor->bytes, alignment, {}); }
            continue;
        }
        if (ranges.empty()) {
