...
When this document and a concrete Op contract differ about that Op's represented inputs, formula,
supported domain, or observable effects, the contract in `include/ninfer/ops/` is the concrete
authority and the inconsistency must be resolved.
...
Preserve only the context needed to interpret the result.
