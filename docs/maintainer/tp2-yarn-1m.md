...
capture and transport probes live in `tools/tp2/`. The reusable evaluation framework and focused
probes remain under `eval/`; raw development-run outputs and machine-specific orchestration scripts
are intentionally excluded from the public Riftline snapshot.
