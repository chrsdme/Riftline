> **Riftline note:** this is inherited NInfer performance documentation. It preserves historical
> measurements and methodology, but it is not the authoritative list of Riftline benchmark claims.
> See [BENCHMARKS.md](BENCHMARKS.md) for the curated public Riftline results. Raw `eval/results/`
> artifacts from the original campaigns are not included in this source snapshot.
...
The original campaign also retained raw probe records, per-request server logs, power and VRAM
samples, and speculative counters. Those raw `eval/results/` artifacts are intentionally excluded
from the public Riftline source snapshot; this document preserves the summarized methodology and
results inherited from the NInfer lineage.
